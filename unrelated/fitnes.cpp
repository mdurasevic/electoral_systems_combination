...
SchedulingEvalOp::SchedulingEvalOp()
{
	...
	Nodes[T_FUTLATES].name = "FUTLATES";
	Nodes[T_FUTLATEL].name = "FUTLATEL";
	Nodes[T_WLATES].name = "WLATES";
	Nodes[T_WLATEL].name = "WLATEL";

	//IDR terminali
	Nodes[T_NLATE].name="NLATE";
	Nodes[T_ISLATE].name = "ISLATE";
	Nodes[T_LATENESS].name = "LATENESS";
	Nodes[T_INDLATE].name = "INDLATE";
	Nodes[T_JOBFINISH].name = "JOBFINISH";
	Nodes[T_TARDINESS].name = "TARDINESS";
	Nodes[T_FLOWTIME].name = "FLOWTIME";
	Nodes[T_INDTARD].name = "INDTARD";
	Nodes[T_INDWTARD].name = "INDWTARD";
...
// originalni dio iz BEAGLE implementacije:
	...
	shortestExecutingJob.Init(sets, max_jobs);
	...
	{
		for(int i=0; i<(uint) N[set][0];i++)
		{
				
			uint minIndex = 0;
			uint minValue = 101;	
			uint nMachines = (uint) Machines[set][0];
			for(uint m = 0; m<nMachines; m++)
			{
				if(minValue>Duration[set][i*nMachines + m])
				{
					minValue = Duration[set][i*nMachines + m];
					minIndex = m;
				}	
			}
			shortestExecutingJob[set][i]= minIndex;
		}
...
*/
	...
	*/
	...
	if(IntUniqueGenotype::IntUniqueGenotype* v = dynamic_cast<IntUniqueGenotype::IntUniqueGenotype*>(individual->getGenotype().get())){
		vector<IndividualP> ensemble;

		for(int i = 0; i<v->intValues.size(); i++)
		{
			ensemble.push_back(individuals[v->intValues[i]]);
		}
...
// oroginalni kod iz BEAGLE implementacije
	...
	ensenbleCount = individuals.size();
...
//			EvaluateUniformVote(dRawFitness);
		...
		break;
		case UNRELATED:
			EvaluateUnrelatedElectionMethodsV2(dRawFitness);
			...
			if(method=="collab" || method=="collab1" ){
				EvaluateEnsembleCollaboration(dRawFitness);
			}
			*/
...
void SchedulingEvalOp::ReadIndividuals(vector<IndividualP> individuals)
{
	...
	for(nTree = 0; nTree<1; nTree++)	// vrtimo kroz stabla
...
void SchedulingEvalOp::CalcStaticTerminals(uint &nNiz, uint &nPoslova, uint &nJob, double &dClock, \
									uint &nMachine, uint &nMachines, double endTime, double trueEnd)
{
...
//	int slCounterM = 0;
	...
	//endtime u uvjet
		...
		{
			pSlack[j] = Deadline[nNiz][j] - (dClock + Duration[nNiz][j*nMachines + nMachine]);
			if(isShortestExecutingJob(nNiz, nMachine, j))
			{
				nShort++;
	...
	if(startInd==-1)
	{
		slackNext = VERY_BIG_VALUE;
		minRelease = VERY_BIG_VALUE;
		startInd = nPoslova;
	} else
	{
		slackNext = pSlack[pIndex[startInd]];
		minRelease = Ready[nNiz][pIndex[startInd]]-dClock;
	}

	if(startIndM==-1)
	{
		slackNextM = VERY_BIG_VALUE;
		minReleaseM = VERY_BIG_VALUE;
	} else
	{
		slackNextM = pSlack[pIndex[startIndM]];
		minReleaseM = Ready[nNiz][pIndex[startIndM]]-dClock;
	}


	double  slackAvg= VERY_BIG_VALUE;
	if(nShort!=0){
		slackAvg = ((double)slacksum)/nShort;
	}
	...
	{	j = pIndex[i];	// 'pravi' indeks posla
		...
		Evaluator.m_dTermValuesArray[T_SLAVGM][j] = slackAvg;
...
//		double tardinessWeight = (Deadline[nNiz][j]<(maxTime+2*Duration[nNiz][j*nMachines + nMachine]))?(WeightT[nNiz][j]*((-Deadline[nNiz][j])+(maxTime+2*Duration[nNiz][j*nMachines + nMachine]))):0;
		...
		if(nrelM!=0){
			Evaluator.m_dTermValuesArray[T_SLAVGD][j] = slavgd/nrelM;
		}else
		{
		...
		Evaluator.m_dTermValuesArray[T_FUTLATES][j] = wTards;
		Evaluator.m_dTermValuesArray[T_WLATES][j] = wLates;

		Evaluator.m_dTermValuesArray[T_FUTLATEL][j] = wtardL;
		Evaluator.m_dTermValuesArray[T_WLATEL][j] = wlateL;
	}
...
inline bool SchedulingEvalOp::isShortestExecutingJob(uint &nNiz, uint &nMachine, uint &index)
{
	return nMachine == shortestExecutingJob[nNiz][index];
}
...
void SchedulingEvalOp::EvaluateSingleSum(double &dRawFitness)
{
	...
	unsigned int nPoslova,nNiz,nJob,i,j,k,index,nLateJobs,nTotalLateJobs=0,nNr;
	unsigned int nLastJob,nOdabrani;
					double dBestArray[50];
				int odabraniArray[50];
	dRawFitness=0;
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				for( ; raspolozivi < nPoslova; raspolozivi++)	// koji je 'najstariji'?
				{	k = pIndex[raspolozivi];
					if(Ready.data[nNiz][k] < kada)	// gledamo najblize vrijeme dolaska
					{	kada = Ready.data[nNiz][k];
						prvi = raspolozivi;
					}
				}	// sada je pIndex[prvi] prvi raspolozivi posao
				...
				if(useLookaheadFactor){
					windowEnd = dClock+(largestReleaseTime-dClock)*lookaheadFactor;
				}
				else{
					int index = nJob;
					int counter = 0;
					while(index<nPoslova && counter < lookaheadJobs)
					{
						if(Ready[nNiz][pIndex[index]]>dClock)
						{
							counter++;
						}
						index++;
					...
					if(index == nJob)
					{
						windowEnd = Ready[nNiz][pIndex[index]];	
					} else{
						windowEnd = Ready[nNiz][pIndex[index-1]];
					}
				}
				...
				for(i=Evaluator.m_iEnd; i<nPoslova && Ready[nNiz][pIndex[i]]<=windowEnd; i++) NULL;
				Evaluator.m_iEnd = i;
				...
				{	//if(MachineReady[nMachine][0] > dClock)	// varijanta samo sa slobodnim strojevima
					//	continue;
					// trebamo izracunati nove vrijednosti vremenski ovisnih terminala!
					CalcTimedTerminals(nNiz,nPoslova,nJob,dClock,nMachine,nMachines, windowEnd);
					CalcStaticTerminals(nNiz,nPoslova,nJob,dClock,nMachine,nMachines,windowEnd, windowEnd);
					Evaluator.m_iPosition = -1;
						Evaluator.m_nTree = 0;
					Evaluator.evaluate_array(pVrijednosti);	// ocijenimo sve
				...
				uint nBestMachine, nBestJobMachine;
				for(i=nJob; i<nPoslova; i++)
				{	if(Ready[nNiz][pIndex[i]] > windowEnd)
						continue;
					// je li posao odabrao neraspolozivi stroj?
					nBestJobMachine = 0;
					for(nMachine=1; nMachine<nMachines; nMachine++)
				...
				if(Ready[nNiz][pIndex[nOdabrani]]>dClock)
				{
					nJob--;
					for(int kk = 0;  kk<nPoslova; kk++)
					{
						if(Ready[nNiz][pIndex[kk]]>dClock)
						{
							dClock = Ready[nNiz][pIndex[kk]];
				...
				{	nJob--;	// azuriraj indeks for petlje (nismo rasporedili posao)
					...
					unsigned int raspolozivi = nJob, prvi = nJob;
					// trebamo pronaci prvog sljedeceg raspolozivog bez nezavrsenih prethodnika
					for(; raspolozivi < nPoslova && (Ready[nNiz][pIndex[raspolozivi]] <= dClock); raspolozivi++)	NULL;
					double kada;
					if(raspolozivi < nPoslova)	// ima li uopce novih?
					{	kada = Ready[nNiz][pIndex[raspolozivi]];	// pocetno vrijeme
						for( ; raspolozivi < nPoslova; raspolozivi++)	// koji je 'najstariji'?
						{	k = pIndex[raspolozivi];
							if(Ready[nNiz][k] < kada && Ready[nNiz][k] > dClock)	// gledamo najblize vrijeme dolaska
							{	kada = Ready[nNiz][k];
								prvi = raspolozivi;
							}
						}	// sada je pIndex[prvi] prvi raspolozivi posao
			...
			{	// pronadjemo prvi raspolozivi stroj
				...
				for(i=nJob; i<nPoslova; i++)	// pa pogledamo ostale
				{	// trazimo najmanju vrijednost
					if(pVrijednosti[pIndex[i]] < dBest)	// je li to najbolja vrijednost?
					{	dBest = pVrijednosti[pIndex[i]];
						nOdabrani = i;
					}
				}
			...
			//		Precedence[j][0] -= 1;
			//	}
			MachineReady[nMachine][0] = dClock  + \
				Duration[nNiz][nOdabrani*nMachines + nMachine];
			// odredimo sljedeci element u matrici rasporeda (indeks stroja je tisucica)
			Schedule[nNiz][nJob] = nOdabrani + nMachine * 1000;
		} // kraj petlje koja vrti poslove u skupu
...
// implementirani terminali: w, dd, pt, SL, pmin, pavg, PAT, MR, age
// ne koristimo u dinamickom: N, Nr, SP, SPr, SD
void SchedulingEvalOp::EvaluateUnrelatedIterativeDR(double &dRawFitness)
{
	double dClock, dRez, dSetFitness, dAvgWeights, dCmax, dTotalCmax=0;
	double dLateness, dTotalLateness=0, dTardiness, dTotalTardiness=0;
	double dNwt, dTotalNwt=0, dBest, dSPr, dSetupTime, dFwt, dTotalFwt=0;
	...
	double NWT = 0;
	double CMAX = 0;
	double drawfitnessold = 10000;
	dTotalTardiness=10000;
	bool flag = true; 
	dCmax = 0;
	Matrica* raspored = &Schedule; 
	Matrica* kopija = NULL;
		int largestReleaseTime = 0;
		double tardin = 10000;
		
		for(int jjj = 0; jjj<60; jjj++)
		{
			count[jjj]=0;
		}
...
// vrtimo sve skupove
	for(nNiz=0; nNiz<sets; nNiz++)
	{	
		double *islate = new double[(int) N.Get(nNiz)];
		double *indLate = new double[(int) N.Get(nNiz)];
		double *jobfinish = new double[(int) N.Get(nNiz)];
		double *indwtard = new double[(int) N.Get(nNiz)];
		double *indtard = new double[(int) N.Get(nNiz)];
		double *flow = new double[(int) N.Get(nNiz)];

		for(i=0; i< (int) N.Get(nNiz); i++){
			islate[i] = 1;
			indLate[i] = 10000;
			indwtard[i] = 10000;
			indtard[i] = 10000;
			...
			flow[i] = 10000;
				...
				nLateJobs = (int) N.Get(nNiz);/*(int) N.Get(nNiz)*/ 
			double drawfitnessold = 10000;
			double latenessOld = 100000;
								int x = 0;
	do{
	...
	// postavljanje pocetnih vrijednosti za pojedini skup
		...
		{	
			...
			//// najkrace trajanje izvodjenja posla
			Evaluator.m_dTermValuesArray[T_pmin][i] = Duration[nNiz][i*nMachines + (uint)PTimeMinMachine[nNiz][i]];
			//Evaluator.m_dTermValuesArray[T_TWT][i] = TWT;
			Evaluator.m_dTermValuesArray[T_NLATE][i] = nLateJobs;
			Evaluator.m_dTermValuesArray[T_ISLATE][i] = islate[i];
			Evaluator.m_dTermValuesArray[T_LATENESS][i] = latenessOld;
			Evaluator.m_dTermValuesArray[T_TARDINESS][i] = tardin;
			Evaluator.m_dTermValuesArray[T_FLOWTIME][i] = flow[i];

			
			Evaluator.m_dTermValuesArray[T_INDLATE][i] = indLate[i];
			Evaluator.m_dTermValuesArray[T_INDWTARD][i] = indwtard[i];
			Evaluator.m_dTermValuesArray[T_INDTARD][i] = indtard[i];
			Evaluator.m_dTermValuesArray[T_JOBFINISH][i] = jobfinish[i];
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				// sat postavimo na najblize vrijeme raspolozivosti
				// sad odredimo koji zadnji posao gledamo iz polja indeksa
				...
				uint nBestMachine, nBestJobMachine;
				for(i=nJob; i<nPoslova; i++)
				{	if(Precedence[pIndex[i]][0] != 0 || Ready[nNiz][pIndex[i]] > windowEnd)
						continue;
					// je li posao odabrao neraspolozivi stroj?
					nBestJobMachine = 0;
					for(nMachine=1; nMachine<nMachines; nMachine++)
				...
				//TODO: maknuti za obicni IDR, vratiti za lookahead!!!!!
				//if(Ready[nNiz][pIndex[nOdabrani]]>dClock)
				//{
				//	nJob--;
				//	for(int kk = 0;  kk<nPoslova; kk++)
		...
		{	if(m_Evaluation)
			...
			{	index = (int) Schedule[nNiz][nJob];
				...
				if(dRez>0){
					indwtard[index] = dRez*WeightT.data[nNiz][index];
					indtard[index] = dRez;
					islate[index]=1;
					nLateJobs++;
					jobfinish[index]=MachineReady[nMachine][0];
					flow[index] = MachineReady[nMachine][0] - Ready[nNiz][index];
				}
				else{
					indtard[index] = 0;
					indwtard[index] = 0;
		...
		//	cout<<latenessOld<<endl;
			//cout<<nLateJobs<<endl;
			if(dRawFitness<drawfitnessold){
				drawfitnessold = dRawFitness;
				latenessOld = dLateness;
				//flow = dFwt;
				tardin = dTardiness;

				//nLateJobs = 0;
				//cout<<dRawFitness<<endl;
				//cout<<"NWT: "<<dNwt<<endl;
 				dRawFitness=0;
				if(kopija!=NULL){
					delete kopija;
				...
				kopija = new Matrica(Schedule);
			}
			else{
				totalRawFitness +=drawfitnessold;
				dRawFitness = 0;
				drawfitnessold = 0;
				//cout<<endl;
				//cout<<totalRawFitness<<endl;
				nTotalLateJobs += 0;
				dTotalNwt += dNwt;
				dTotalFwt += dFwt;
				dTotalLateness += dLateness;
...
void SchedulingEvalOp::copyRow(Matrica &M1, Matrica &M2, int row, int numEl)
{
...
void SchedulingEvalOp::copyMachineReady(Matrica &M1, Matrica &M2, unsigned int row)
{
	for(int i = 0; i<row; i++)
	{
		M1[i][0] = M2[i][0];
...
void SchedulingEvalOp::EvaluateUnrelatedRollout(double &dRawFitness)
{
	 ...
	 newSchedule.Init(sets, max_jobs);
	 newMachineReady.Init(sets, max_jobs);
	 previousSchedule.Init(sets, max_jobs);
	 previousMachineReady.Init(sets, max_jobs);
	double dClock, dRez, dSetFitness, dAvgWeights, dCmax, dTotalCmax=0;
	double dLateness, dTotalLateness=0, dTardiness, dTotalTardiness=0;
	double dNwt, dTotalNwt=0, dBest, dSPr, dSetupTime, dFwt, dTotalFwt=0;
...
//	cout<<nNiz;
	// postavljanje pocetnih vrijednosti za pojedini skup
		...
		for(nJob=0; nJob<nPoslova; nJob++){	// rasporedjujemo svaki posao unutar skupa
				...
				// sad odredimo koji zadnji posao gledamo iz polja indeksa
				for(i=prvi; i<nPoslova && Ready[nNiz][pIndex[i]]<=windowEnd; i++) NULL;
				Evaluator.m_iEnd = i;
				zadnji = i;
				...
				for(nMachine=0; nMachine<nMachines; nMachine++)
				{
					...
					for(unsigned int jj = nJob; jj<nPoslova && Ready[nNiz][pIndex[jj]]<=rolloutEnd; jj++)
					{
						
						// prepisivanje u nove
						double fitnessValue = 0;
						unsigned int *novipIndex = new unsigned int[nPoslova];
						copyMachineReady(newMachineReady, MachineReady, nMachines);
						//copyRow(newMachineReady, MachineReady, nNiz, nMachines);
						copyRow(newSchedule, Schedule, nNiz, nPoslova);

						//newMachineReady = Matrica(MachineReady);
						//newSchedule = Matrica(Schedule);
			...
			uint nextJob = nJob+1;
			cClock = dOldClock;
			//EvaluateUnrelatedRolloutH(fitnessValue, nNiz, lookaheadFactor, largestReleaseTime, newSchedule, newMachineReady, nextJob);
		...
		if(previousBestPriority<priority || nJob<=nPoslova)
		{
			//Schedule = Matrica(previousSchedule);
			//MachineReady = Matrica(previousMachineReady);
			copyRow(Schedule, previousSchedule, nNiz, nPoslova);
//			copyRow(MachineReady, previousMachineReady, nNiz, nMachines);
						copyMachineReady(MachineReady, previousMachineReady, nMachines);
		...
		Evaluator.m_iEnd = 1;	// pocetna vrijednost zadnjeg posla koji se uzima u o obzir
			...
			MachineReady[nBestMachine][0] = dClock  + \
				Duration[nNiz][nOdabrani*nMachines + nBestMachine];
			// odredimo sljedeci element u matrici rasporeda (indeks stroja je tisucica)
			Schedule[nNiz][nJob] = nOdabrani + nBestMachine * 1000;
			...
			copyRow(previousSchedule, Schedule, nNiz, nPoslova);
			//copyRow(previousMachineReady, MachineReady, nNiz, nMachines);
			copyMachineReady(previousMachineReady, MachineReady, nMachines);
...
// vrtimo sve skupove
		...
		{	
			if(Ready[nNiz][pIndex[nJob]]>cClock){
				zadnji = nJob;
				break;
			}
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				{	if(Ready[nNiz][pIndex[i]] > windowEnd)
					...
					//cout<<Values[nBestMachine][pIndex[i]]<<endl;
				}
			...
			//ENS -> ako koristimo rasporedivanje samo prvog posla.
			if(method=="collab1" && scheduled == nJobsCollab){	
				//zadnji = nJob+1;
				break;
			}
...
// implementirani terminali: w, dd, pt, SL, pmin, pavg, PAT, MR, age
// ne koristimo u dinamickom: N, Nr, SP, SPr, SD
void SchedulingEvalOp::EvaluateUnrelatedRolloutIterative(double &dRawFitness)
{
...
//	cout<<nNiz;
	// postavljanje pocetnih vrijednosti za pojedini skup
		...
		for(i=0; i< (int) N.Get(nNiz); i++){
			bestislate[i] = 1;
			bestindLate[i] = 10000;
			bestindwtard[i] = 10000;
			bestindtard[i] = 10000;
			bestjobfinish[i] = 10000;
			bestflow[i] = 10000;
		}
		bestNLateJobs = (int) N.Get(nNiz);
		bestLateness = 100000;
		bestTardiness = 100000;
		...
		for(nJob=0; nJob<nPoslova; nJob++){	// rasporedjujemo svaki posao unutar skupa
				...
				if(useRolloutFactor){
					rolloutEnd = dClock+(largestReleaseTime-dClock)*rolloutFactor;
				}
				else{
					int indexR = nJob;
					int counterR = 0;
					while(indexR<nPoslova && counterR < rolloutJobs)
					{
						if(Ready[nNiz][pIndex[indexR]]>dClock)
						{
							counterR++;
						}
						indexR++;
					...
					if(indexR==nJob)
					{
						rolloutEnd = Ready[nNiz][pIndex[indexR]];
					} else
						rolloutEnd = Ready[nNiz][pIndex[indexR	-1]];
				}
				if(rolloutEnd< dClock)
					rolloutEnd = dClock;
				
				/*cout<<dClock+(largestReleaseTime-dClock)*rolloutFactor<<endl;
				cout<<rolloutEnd<<endl;
				cout<<endl;*/
				...
				for(nMachine=0; nMachine<nMachines; nMachine++)
				{
					...
					for(unsigned int jj = nJob; jj<nPoslova && Ready[nNiz][pIndex[jj]]<=rolloutEnd; jj++)
					{
						...
						pIndex[nJob] = i;
			uint nextJob = nJob+1;
			EvaluateUnrelatedRolloutHIterative(fitnessValue, nNiz, lookaheadFactor, largestReleaseTime, newSchedule, newMachineReady, nextJob);
		...
		Evaluator.m_iEnd = 1;	// pocetna vrijednost zadnjeg posla koji se uzima u o obzir
			...
			unsigned int raspolozivi = nJob; //prvi = nJob;
				...
				{	//if(MachineReady[nMachine][0] > dClock)	// varijanta samo sa slobodnim strojevima
					...
					// trebamo izracunati nove vrijednosti vremenski ovisnih terminala!
					CalcTimedTerminals(nNiz,nPoslova,nJob,dClock,nMachine,nMachines, MachineReady);
					//CalcStaticTerminals(nNiz,nPoslova,nJob,dClock,nMachine,nMachines,dClock, windowEnd);
					Evaluator.m_iPosition = -1;
						Evaluator.m_nTree = 0;
					Evaluator.evaluate_array(pVrijednosti);	// ocijenimo sve
					memcpy(Values[nMachine],pVrijednosti,nPoslova*sizeof(double));
			...
			previousClock = dClock;
								...
								for(i=0; i< (int) N.Get(nNiz); i++){
bestislate[i]=			bestislate[i];
			bestindLate[i]= llbestindLate[i];
			bestindwtard[i]=llbestindwtard[i];
			bestindtard[i]=llbestindtard[i];
			bestjobfinish[i]=llbestjobfinish[i];
			bestflow[i]=llbestflow[i];
		}
		bestNLateJobs = llbestNLateJobs;
		bestLateness = llbestLateness;
		bestTardiness = llbestTardiness;	
...
// implementirani terminali: w, dd, pt, SL, pmin, pavg, PAT, MR, age
// ne koristimo u dinamickom: N, Nr, SP, SPr, SD
void SchedulingEvalOp::EvaluateUnrelatedRolloutHIterative(double &dRawFitness, unsigned int nNiz, double lookahead, int &largestReleaseTime, Matrica &Schedule, Matrica &MachineReady, unsigned int &prvi)
{
	double dClock, dRez, dSetFitness, dAvgWeights, dCmax, dTotalCmax=0;
	double dLateness, dTotalLateness=0, dTardiness, dTotalTardiness=0;
	double dNwt, dTotalNwt=0, dBest, dSPr, dSetupTime, dFwt, dTotalFwt=0;
	...
	Matrica MReadyBack = MachineReady;

		double tardin = bestTardiness;
		bool firstStep = true;
		int bestScheduledComb;
...
// vrtimo sve skupove
		...
		for(i=0; i< (int) N.Get(nNiz); i++){
			islate[i] = bestislate[i];
			indLate[i] = bestindLate[i];
			indwtard[i] = bestindwtard[i];
			indtard[i] = bestindtard[i];
			jobfinish[i] = bestjobfinish[i];
			flow[i] = bestflow[i];
		}
				nLateJobs = bestNLateJobs;
			double latenessOld = bestLateness;
								int x = 0;
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				if(useLookaheadFactor){
					...
					int index1 = nJob;
					int counter = 0;
					while(index1<nPoslova && counter < lookaheadJobs)
					{
						if(Ready[nNiz][pIndex[index1]]>dClock)
						{
							counter++;
						}
						index1++;
					...
					if(index1 == nJob)
					{
						windowEnd = Ready[nNiz][pIndex[index1]];	
					} else{
						windowEnd = Ready[nNiz][pIndex[index1-1]];
					}
				}
												...
												if(firstStep)
		{
			...
			MachineReady[nMachine][0] = dClock  + \
				Duration[nNiz][nOdabrani*nMachines + nMachine];
			// odredimo sljedeci element u matrici rasporeda (indeks stroja je tisucica)
			//cout<<"dclock "<<dClock<<endl;
			//cout<<"M1 "<<MachineReady[0][0]<<endl;
			//cout<<"M2 "<<MachineReady[1][0]<<endl;
			//cout<<"M3 "<<MachineReady[2][0]<<endl;
			//cout<<"j "<<nOdabrani<<endl;
			//cout<<"m "<<nMachine<<endl;
			
			if(firstStep)
			{
				firstStep = false;
				bestScheduledComb = nOdabrani + nMachine * 1000;
		...
		{	if(m_Evaluation)
			...
			if(dRawFitness<drawfitnessold){
				...
				if(once)
				{
					once = false;
							for(i=0; i< (int) N.Get(nNiz); i++){
			lllbestislate[i] = islate[i];
			lllbestindLate[i] = indLate[i];
			lllbestindwtard[i] = indwtard[i];
			lllbestindtard[i] = indtard[i];
			lllbestjobfinish[i] = jobfinish[i];
			lllbestflow[i] = flow[i];
		}
		lllbestNLateJobs = nLateJobs;
		lllbestLateness = latenessOld;
		lllbestTardiness = tardin;
				}
		...
		lllbestTardiness = tardin;
				...
				if(bestbestbest>=drawfitnessold)
				{
					bestbestbest=drawfitnessold;

							for(i=0; i< (int) N.Get(nNiz); i++){
llbestislate[i]=			lllbestislate[i];
			llbestindLate[i]= lllbestindLate[i];
			llbestindwtard[i]=lllbestindwtard[i];
			llbestindtard[i]=lllbestindtard[i];
			llbestjobfinish[i]=lllbestjobfinish[i];
			llbestflow[i]=lllbestflow[i];
		}
		llbestNLateJobs = lllbestNLateJobs;
		llbestLateness = lllbestLateness;
		llbestTardiness = lllbestTardiness;
				}


				//bestover = bestScheduledComb;
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				uint nBestMachineArray[50];
				uint nBestJobMachineArray[50];
...
void SchedulingEvalOp::EvaluateUnrelatedElectionMethods(double& dRawFitness)
{
	double dClock, dRez, dSetFitness, dAvgWeights, dCmax, dTotalCmax = 0;
	double dLateness, dTotalLateness = 0, dTardiness, dTotalTardiness = 0;
	double dNwt, dTotalNwt = 0, dBest, dSPr, dSetupTime, dFwt, dTotalFwt = 0;
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				uint nBestMachine, nBestJobMachine;
				int pindex;
				//BaldwinMethod electionMethod;

				for (i = nJob; i < nPoslova; i++)
				{
					if (Ready[nNiz][pIndex[i]] > dClock)
						continue;
					nBestJobMachine = 0;
					...
					for (nMachine = 0; nMachine < nMachines; nMachine++) {
						for (int kk = 0; kk < ensenbleCount; kk++) {
							if(nMachine==0)
							{
								priorities.push_back(vector<double>());
							}
							//printf("%d", kk);
							priorities[kk].push_back ((ValuesArray[kk])[nMachine][pIndex[i]]);
						}
					}
					
					shared_ptr<WinnerStrategy> winnerStrategy = make_shared<HighestPriorityStrategy>();
					int winner = electoralSystem->elect(priorities, winnerStrategy);
					nBestJobMachine = winner;
					...
					else	// inace vidi kolika je vrijednost
					{
						...
						for (int kk = 0; kk < ensenbleCount; kk++) {
							priorities2.push_back(vector<double>());
							priorities2[kk].push_back(ValuesArray[kk][nBestJobMachine][pIndex[i]]);
							priorities2[kk].push_back(ValuesArray[kk][nBestMachine][pindex]);
						}

						shared_ptr<WinnerStrategy> winnerStrategy = make_shared<LowestPriorityStrategy>();
						int chosen = electoralSystem->elect(priorities2, winnerStrategy);
						if (chosen == 0) {
							pindex = pIndex[i];
							nBestMachine = nBestJobMachine;
							nOdabrani = i;
...
void SchedulingEvalOp::EvaluateUnrelatedElectionMethodsV2(double& dRawFitness)
{
	double dClock, dRez, dSetFitness, dAvgWeights, dCmax, dTotalCmax = 0;
	double dLateness, dTotalLateness = 0, dTardiness, dTotalTardiness = 0;
	double dNwt, dTotalNwt = 0, dBest, dSPr, dSetupTime, dFwt, dTotalFwt = 0;
...
// 
			{	// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				...
				uint nBestMachine, nBestJobMachine;
				int pindex;
				//TidemanAlternativeSmith    electionMethod;
				//electionMethod;

				vector<int> jobIndices;
				vector<int> machineIndices;
				...
				for (i = nJob; i < nPoslova; i++)
				{
					...
					if (MachineReady[nBestJobMachine][0] > dClock)
						continue;
					BestSet = true;
					jobIndices.push_back(i);
					machineIndices.push_back(nBestJobMachine);
				}
				...
				if (BestSet) {
					...
					{
						priorities.push_back(vector<double>());
						for (int jj = 0; jj < jobIndices.size(); jj++)
						{
							int jobpIndex = pIndex[jobIndices[jj]];
							int machineIndex = machineIndices[jj];
							priorities[kk].push_back(ValuesArray[kk][machineIndex][jobpIndex]);
						}
					}

					shared_ptr<WinnerStrategy> winnerStrategy = make_shared<LowestPriorityStrategy>();
					int chosen = electoralSystem->elect(priorities, winnerStrategy);

					pindex = pIndex[jobIndices[chosen]];
					nBestMachine = machineIndices[chosen];
					nOdabrani = jobIndices[chosen];
				}
...
// implementirani terminali: w, dd, pt, SL, pmin, pavg, PAT, MR, age
// ne koristimo u dinamickom: N, Nr, SP, SPr, SD
void SchedulingEvalOp::EvaluateEnsembleCollaboration(double &dRawFitness)
{
	occur.resize(50);
	for(int i = 0; i<50; i++){
		occur[i] = 0;
...
// 
			...
			{	
				...
				// PROMJENA
					for(int kk = 0; kk<ensenbleCount; kk++){
						...
						ruleUsed = kk;
						cClock = dClock;
						EvaluateUnrelatedRolloutH(fitnessValue, nNiz, 0, largestReleaseTime, newSchedule, newMachineReady, nextJob);
						...
						delete[] newPlastJob;
												...
												{
													//cout<<chosenRule<<endl;
													occur[chosenRule]++;
													Evaluator.m_iPosition = -1;
				Evaluator.m_iOffset = nJob;										
				// trebamo naci prvi raspolozivi stroj i prvi raspolozivi posao
				unsigned int raspolozivi = nJob, prvi = nJob;
				// trebamo pronaci prvog raspolozivog bez nezavrsenih prethodnika
				for(; Precedence[pIndex[raspolozivi]][0] > 0; raspolozivi++)	NULL;
	...
	void SchedulingEvalOp::SetElectoralSystem(shared_ptr<ElectoralSystem> electoralSystem)
	{
...
// 
	...
	Fitness[sets][Fwt] = dTotalFwt;
	Fitness[sets][Cmax] = dTotalCmax;
	//cout<<Fitness[sets][Twt]<<" "<<Fitness[sets][Nwt]<<" "<<Fitness[sets][Cmax]<<endl;
}
